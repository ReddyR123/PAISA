  ...
  if (req.method === 'GET') {
    ...
    try {
      const t = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent`, {
        ...
        body: JSON.stringify({ contents: [{ parts: [{ text: 'Return this JSON exactly: {"ok":true}' }] }], generationConfig: { responseMimeType: 'application/json', maxOutputTokens: 8192, thinkingConfig: { thinkingBudget: 0 } } })
      });
